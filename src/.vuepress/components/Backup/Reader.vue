<template>
	<span>
		...
		<span v-else>
			...
			<div class="guide">
				<p class="title">
					<ElButton type="danger" size="mini" icon="el-icon-close" circle @click="closeBackup()"></ElButton>
					{{ backupFilename }}
				</p>
				...
				<span class="filters-list">
					...
					<div v-if="Object.keys(backup.tabs).length" class="filter tabsOption">
						...
						<ElSelect v-model="filters.tabs" placeholder="All tabs" multiple clearable>
							<ElOption v-for="(name, id) in backup.tabs" :key="id" :label="name" :value="id" />
						</ElSelect>
					</div>
					...
					<div v-if="Object.keys(backup.sources).length" class="filter sourcesOption">
						...
						<ElSelect v-model="filters.sources" placeholder="All sources" multiple clearable>
							<ElOption v-for="(name, id) in backup.sources" :key="id" :label="name" :value="id" />
						</ElSelect>
					</div>
				</span>
			</div>
		...
		<div class="backupContainer">
			<span v-for="manga in filtredLibrary" :key="manga.id" class="mangaTile" @click="showManga(manga)">
				<Backup-MangaTile :manga="manga" />
			</span>
		</div>

		<!-- Manga dialog -->
		<ElDialog v-if="dialog.visible" :visible.sync="dialog.visible" :title="dialog.mangaObject.titles[0]" center>
...
export default {
	...
	computed: {
		...
		filtredLibrary() {
			...
			if (filters.search) {
				library = library.filter((manga) => this.includeInTitles(manga.titles, filters.search));
			}
